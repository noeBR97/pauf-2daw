...
def crear_personaje():
    ...
    while aux_is_heroe_str.upper() not in ["SI", "S", "NO", "N"]:
        aux_is_heroe_str = input("¿Quieres crear un heroe? Introduce si/no")
        if aux_is_heroe_str.upper() in (["SI", "S"]):
            is_heroe_bl = True
    ...
    if is_heroe_bl:
        ...
        registrar_heroe_en_json(pj)
    else:
        pj = Villano(nombre_str, apellidos_str, fecha_str)
        registrar_villano_en_json(pj)
    return pj
...
def registrar_heroe_en_json(pj):
    nuevos_datos_python = []
    ...
    with open("./archivos_json/heroes.json", 'w') as archivo:
        json.dump(nuevos_datos_python, archivo, indent=4)

def registrar_villano_en_json(pj):
    nuevos_datos_python = []
